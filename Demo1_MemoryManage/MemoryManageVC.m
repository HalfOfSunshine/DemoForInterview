...
//2，僵尸对象：一个OC对象引用计数为0被释放后就变成僵尸对象了    //（废话）：僵尸对象的内存已经被系统回收，虽然可能该对象还存在，数据依然在内存中，但僵尸对象已经是不稳定对象了，不可以再访问或者使用，它的内存是随时可能被别的对象申请而占用的。
...
-(void)testNilPointer{
    NilPointerObj *obj = [NilPointerObj alloc];//分配内存
    NSLog(@"精神小伙第一次出现");
    NSLog(@"obj指向：%p",obj);
    [obj takeInterview];
    obj = [obj init];//创建实例对象
